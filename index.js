...
// rachel
function wordMatch(inputWord) {
  if (inputWord == answerWord) console.log("CONGRATS YOU WON!!!");
...
function letterMatch(str) {
  ...
  for (let i = 0; i < answerWord.length; i++) {
    if (str[i] == answerWord[i]) {
      processedWord += chalk.green(str[i]);
      continue;
    }
    if (answerWord.includes(str[i])) {
      processedWord += chalk.yellow(str[i]);
    } else {
      processedWord += chalk.grey(str[i]);
    }
  }
